...
class Form extends Component {
	...
	render() {
		...
		const { title, slug, images, currentLinkKey: current_link_key, publish, statusSaving: status_saving } = this.state;
		const { 
			...
			<>
				<nav className="bg-white border-b-2 border-indigo-600 top-0 shadow mb-4 py-3 fixed left-0 w-full z-10">
				    <div className="container mx-auto sm:px-6 px-4 md:px-0">
				        <div className="flex items-center{{ !isset($empty_navbar) ? ' justify-center' : ''}}">
