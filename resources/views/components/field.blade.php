<div class="inline-block w-full mb-5">
	<label class="mb-2 inline-block text-sm text-gray-600 {{ isset($error) ? 'text-red-600' : '' }}" for="{{ $name }}">{{ $label }}</label>
	@isset($type)
		@if($type == 'textarea')
			<textarea name="{{ $name }}" class="w-full inline-block bg-gray-100 border {{ isset($error) ? 'border-red-600' : '' }} py-2 px-4 rounded focus:bg-white outline-none h-32 text-sm">{{$value ?? ''}}</textarea>
		...
		@elseif($type == 'select')
			{!! Form::select($name, $data ?? [], $value ?? null, ['class' => 'w-full inline-block bg-gray-100 border ' . (isset($error) ? 'border-red-600' : '') . ' h-12 px-4 rounded focus:bg-white outline-none ' . ($class ?? ''), 'id' => $id ?? '', isset($multiple) ? 'multiple' : '']) !!}
		@elseif($type == 'div')
		<div id="{{ $name }}" class="w-full inline-block bg-gray-100 border {{ isset($error) ? 'border-red-600' : '' }} py-2 px-4 rounded focus:bg-white outline-none {{ $class ?? '' }}">{{ $value ?? '' }}</div>
		@else
		<input id="{{ $name }}" class="w-full inline-block bg-gray-100 border {{ isset($error) ? 'border-red-600' : '' }} py-2 px-4 rounded focus:bg-white outline-none {{ $class ?? '' }}" type="{{ $type }}" name="{{ $name }}" value="{{ $value ?? '' }}">
		@endif
	@endisset
