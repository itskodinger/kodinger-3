...
class Form extends Component {
	...
	addDOMFunctionality({defaultTags}) {
		setTimeout(() => {
	...
	uploadImage(image) {
		...
		.catch((error) => {
			...
			this.removeImage(image.id, true);
		});
	}
				    ...
				    <div className="container mx-auto sm:px-6 px-4 md:px-0">
				        <div className="flex items-center justify-center">
				            ...
				            <div className="ml-auto flex items-center">
				            	<div className="text-gray-600 text-sm mr-6 save-status">
				            		{statusSaving}
				            	</div>
				            	<button onClick={this.publishWholeContent.bind(this)} className={`items-center bg-gradient text-white px-4 py-2 text-sm rounded mr-6 shadow-md hover:shadow-none flex` + (!publish || this.isUploadingImage() ? ' pointer-events-none opacity-50' : '')}>
				            		Publish Post
				            	</button>
				            </div>
			            </div>
										...
										<div className="dropzone rounded-lg border-2 border-dashed border-gray-300 w-full flex items-center justify-center">
											<div className="p-20 text-center">
												...
												<p className="text-sm mt-2 text-gray-600">
													Maksimal: 5MB. Format yang didukung: {[...this.allowedImageTypesReadable, ...this.allowedVideoTypesReadable].join(', ')}
												</p>
