...
class Form extends Component {
				...
				if(err && !err.ok) {
					switch(err.status) {
						case 422:
							...
							this.toast.add(`😠&nbsp; ${firstError.replace(/pages/, 'link')}`);
						break;
	...
	publishPost() {
		const { link: pages } = this.state;

		const rawTags = this.tagify.value;
		let tags = '';
				    ...
				    <div className="container mx-auto sm:px-6 px-4 md:px-0">
				        <div className="flex items-center justify-center">
				            ...
				            <div className="ml-auto flex items-center">
				            	<button onClick={this.publishPost.bind(this)} className={`items-center bg-indigo-600 text-white px-4 py-2 rounded mr-6 flex ${this.state.progress ? 'opacity-50 pointer-events-none' : ''}`}>
				            		Publish Post
				            	</button>
				            </div>
			            </div>
	                    ...
	                    <div className="w-full lg:w-6/12 px-4 md:w-8/12">
	                    	{this.state.success &&
			                    <div className="py-4 px-6 mb-4 bg-teal-100 text-teal-600 border border-teal-200 rounded leading-loose">
			                    	Tautan kamu sudah berhasil dibagikan, orang lain dapat mengaksesnya. <a href={routes.single + user.username} className="border-b border-teal-600 pb-1 font-semibold">Pergi ke profile</a> atau <a href={routes.post_new} className="border-b border-teal-600 pb-1 font-semibold">buat post baru</a>
			                    </div>
			                }
			            	<div className="border border-gray-200 p-6 md:p-8 rounded">
			                    <h1 className="text-indigo-600 text-xl font-semibold">Bagikan Tautan Eksternal</h1>
			                    <p className="mb-4 mt-2 text-gray-600">Kamu menulis artikel di blog pribadi kamu? Atau menemukan sesuatu yang keren di internet, bagikan tautannya di sini.</p>

			            		<div className="mb-6 mt-6">
			            			<label className="mb-1 inline-block text-gray-600">Tautan</label>
			            			<input onChange={this.linkOnInput.bind(this)} type="text" name="link" className="text-gray-600 border border-gray-200 rounded block w-full py-3 px-5 focus:outline-none focus:border-indigo-600" autoComplete="off" tabIndex="1" placeholder="https://blogkamu.com/tutorial-figma" />
			            		</div>
